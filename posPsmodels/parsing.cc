...
//TODO: change the parse according the new syntax, ie an ordered disjunction rule
// is now alpha_1 c_1 x alpha_2 c_2 x ... x alpha_k c_k :- B+, not B-. instead of
// alpha c_1 x c_2 x ... x c_k :- B+, not B-.
int
	...
	while (getline(f, line)) {
		...
		if (line.empty()) {
			...
			default:
				...
				//TODO change here for the new syntax
...
generatePossibilisticModels(list<LppodModel> *possModelList, string possmodel,LpodModel *lpodModel) {
	...
	//lpodModel->printAtoms();
	//lpodModel->printSatisfactionDegree();
	//cout << lpodModel->getModel_id() << endl;
...
generateLppod(LppodProgram *lppod,string program_name) {
	...
	//out << i;
	//nr = out.str();
	transformedLppod = Utils::string2char("tmp/"+program_name);
