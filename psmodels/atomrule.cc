  ...
  : p(p0)
{
  ...
  /*** TSS 2.1.2001 **/
  /*** TSS 28.5.2001 **/
  branch_merge_tested  = false;
  sometimes_true = false;
  sometimes_false = false;
  /*** TSS 28.5.2001 **/
}
...
void
BasicRule::print_internal (ostream &os)
{
  os << (int)type << ' ' << head->posScore << ' '
...
void
ConstraintRule::print_internal (ostream &os)
{
  os << (int)type << ' ' << head->posScore << ' '
       << (nend-nbody)+(pend-pbody) << ' ' << nend-nbody;
  head->negScore = 1;
...
void
GenerateRule::print_internal (ostream &os)
{
...
void
ChoiceRule::print_internal (ostream &os)
{
  if (head == hend)
    return;
  os << (int)type << ' ' << hend-head;
  Atom **a;
  for (a = head; a != hend; a++)
    {
      os << ' ' << (*a)->posScore;
  ...
  os << ' ' << (nend-nbody)+(pend-pbody) << ' ' << nend-nbody;
  for (a = nbody; a != nend; a++)
    {
      os << ' ' << (*a)->posScore;
      (*a)->negScore = 1;
...
void
WeightRule::print_internal (ostream &os)
{
# ifdef USEDOUBLE
...
void
OptimizeRule::print_internal (ostream & os)
{
# ifdef USEDOUBLE
